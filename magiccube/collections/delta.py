...
from magiccube.collections.cubeable import BaseCubeable, CubeableCollection
...
class CubeDelta(object):
    ...
    def new_printings(self) -> FrozenMultiset[Printing]:
        if self._new_printings is None:
            self._new_printings = FrozenMultiset(self._current.all_printings) - FrozenMultiset(
                self._original.all_printings
            )
    ...
    def removed_printings(self) -> FrozenMultiset[Printing]:
        if self._removed_printings is None:
            self._removed_printings = FrozenMultiset(self._original.all_printings) - FrozenMultiset(
                self._current.all_printings
            )
    ...
    def report(self) -> str:
        return (
            f"New cubeables ({len(self.new_cubeables)}):\n{self.new_cubeables.pp_string}\n------\n"
    ...
    def as_operation(self) -> CubeDeltaOperation:
        return CubeDeltaOperation(self._current.cubeables.elements()) - CubeDeltaOperation(
...
class CubeDeltaOperation(CubeableCollection, PersistentHashable):
    def __init__(
        ...
        self._cubeables: FrozenCounter[Cubeable] = FrozenCounter() if cubeables is None else FrozenCounter(cubeables)
    ...
    def printings(self) -> t.Iterator[t.Tuple[Printing, int]]:
        return (
            ...
            for cubeable, multiplicity in self._cubeables.items()
            if isinstance(cubeable, Printing)
        )

    @property
    def new_printings(self) -> t.Iterator[t.Tuple[Printing, int]]:
        return ((cubeable, multiplicity) for cubeable, multiplicity in self.printings if multiplicity > 0)
    ...
    def new_traps(self) -> t.Iterator[t.Tuple[Trap, int]]:
        return ((trap, multiplicity) for trap, multiplicity in self.traps if multiplicity > 0)
    ...
    def new_tickets(self) -> t.Iterator[t.Tuple[Ticket, int]]:
        return ((ticket, multiplicity) for ticket, multiplicity in self.tickets if multiplicity > 0)
    ...
    def new_purples(self) -> t.Iterator[t.Tuple[Purple, int]]:
        return ((purple, multiplicity) for purple, multiplicity in self.purples if multiplicity > 0)
    ...
    def laps(self) -> t.Iterator[t.Tuple[Lap, int]]:
        return (
            (cubeable, multiplicity) for cubeable, multiplicity in self._cubeables.items() if isinstance(cubeable, Lap)
        )
    ...
    def new_cubeables(self) -> t.Iterator[t.Tuple[Cubeable, int]]:
        return ((cubeable, multiplicity) for cubeable, multiplicity in self._cubeables.items() if multiplicity > 0)
    ...
    def _calc_persistent_hash(self) -> t.Iterable[t.ByteString]:
        ...
        for persistent_hash, multiplicity in sorted(
            ((lap.persistent_hash(), multiplicity) for lap, multiplicity in self.laps),
    ...
    def __add__(self, other: t.Union[CubeDeltaOperation, Cube]) -> CubeDeltaOperation:
        return self.__class__(self._cubeables + other.cubeables)
    ...
    def __invert__(self):
        return self.__class__(self._cubeables * -1)
