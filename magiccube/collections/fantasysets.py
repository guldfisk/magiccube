...
from magiccube.collections.cube import BaseCube, CardboardCube, Cube
...
class BaseFantasySet(Serializeable, PersistentHashable, t.Generic[C]):
    ...
    def serialize(self) -> serialization_model:
        return {"type": self.__class__.__name__, "rarity_map": {k: v.serialize() for k, v in self._rarity_map.items()}}
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, self.__class__) and self._rarity_map == other._rarity_map
    ...
    def __repr__(self) -> str:
        return "{}({})".format(
            ...
            ", ".join("{}: {}".format(rarity, len(cube)) for rarity, cube in self._rarity_map.items()),
        )
...
class CardboardFantasySet(BaseFantasySet[CardboardCube]):
    ...
    def deserialize(cls, value: serialization_model, inflator: Inflator) -> CardboardFantasySet:
        return cls({k: CardboardCube.deserialize(v, inflator) for k, v in value["rarity_map"].items()})


class FantasySet(BaseFantasySet[Cube]):
    @property
    def as_cardboards(self) -> CardboardFantasySet:
        return CardboardFantasySet({rarity: cards.as_cardboards for rarity, cards in self._rarity_map.items()})

    @classmethod
    def deserialize(cls, value: serialization_model, inflator: Inflator) -> FantasySet:
        return cls({k: Cube.deserialize(v, inflator) for k, v in value["rarity_map"].items()})
...
class FantasyBoosterKeySlot(Serializeable, PersistentHashable):
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, self.__class__) and self._key_map == other._key_map
    ...
    def __repr__(self) -> str:
        return "{}({})".format(
            self.__class__.__name__,
            ", ".join("{}: {}".format(key, weight) for key, weight in self._key_map.items()),
        )
...
class FantasyBoosterKey(Serializeable, PersistentHashable):
    ...
    def get_booster_map(self, fantasy_set: FantasySet) -> BoosterMap[Cubeable]:
        return BoosterMap({slot.get_map_slot(fantasy_set): multiplicity for slot, multiplicity in self.slots.items()})

    def serialize(self) -> serialization_model:
        return {
            "type": self.__class__.__name__,
            "slots": [(slot.serialize(), multiplicity) for slot, multiplicity in self._slots.items()],
        }
    ...
    def deserialize(cls, value: serialization_model, inflator: Inflator) -> FantasyBoosterKey:
        return cls(
            FrozenMultiset(
                ...
                for slot, multiplicity in value["slots"]
            )
        )
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, self.__class__) and self._slots == other._slots

    def _calc_persistent_hash(self) -> t.Iterator[t.ByteString]:
        for _hash, multiplicity in sorted(
            ((slot.persistent_hash(), multiplicity) for slot, multiplicity in self._slots.items()),
            key=lambda p: p[0],
        ):
